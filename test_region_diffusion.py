"""
Test Region-Based Diffusion Detection
"""

import os
from dotenv import load_dotenv
from src.analyzers.region_diffusion_detector import analyze_document_regions_with_diffusion, visualize_diffusion_regions
...
def main():
    print("="*80)
    print(" REGION-BASED DIFFUSION DETECTION TEST")
    print("="*80)
    
    output_dir = "verification_output_region_diffusion"
    ...
    for idx, doc in enumerate(documents, 1):
        ...
        output_path = os.path.join(doc_dir, "diffusion_region_analysis.png")
        
        # Run region-based Diffusion detection
        result = analyze_document_regions_with_diffusion(doc['path'])
        ...
        if result.get('suspicious_regions'):
            ...
            for sr in result['suspicious_regions']:
                print(f"      • {sr['type']}: {sr['description']} (Diffusion: {sr['confidence']:.0%})")
        ...
        # Visualize
        visualize_diffusion_regions(doc['path'], result, output_path)
