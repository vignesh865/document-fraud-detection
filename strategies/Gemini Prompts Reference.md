...
Both prompts benefit from file-hash based caching:
...
cache_key = compute_file_hash(image_path)
...
**Benefits**:
- Reduces API costs (repeated analysis)
- Faster re-analysis
...
- **Use when**: Need semantic understanding, definitive proofs

### Segmentation
- **Model**: gemini-2.0-flash-exp (cheaper)
- **Typical cost**: ~$0.001-0.005 per image
- **Use when**: Need region identification for downstream analysis

### Optimization
- Cache aggressively
- Use segmentation + cheap CV instead of multiple forensic calls
- Batch when possible
- Use flash model for segmentation, pro for critical decisions
